<template>
  <div class="area-chart-container">
    ...
    <div class="chart-canvas">
      <svg :width="width" :height="height" class="area-chart">
        ...
        <!-- Data Areas -->
        <g class="areas-layer">
          <g v-for="key in keys" :key="`area-${key}`">
            <!-- Area Path -->
            <path 
              :d="generateAreaPath(key)"
              :fill="getAreaFill(key)"
              :opacity="key === selectedKey || !selectedKey ? 0.5 : 0.2"
            />
...
const dataExtent = computed(() => {
  if (props.stacked) {
    // For stacked areas, find the maximum sum of all values at each data point
    const maxSums = props.data.map(item => {
      return props.keys.reduce((sum, key) => {
        const val = item[key];
        return sum + (typeof val === 'number' ? val : 0);
      }, 0);
    });
    
    const min = 0; // Stacked areas always start from 0
    const max = Math.max(...maxSums);
    // Add a bit of padding to the top
    const paddedMax = max + max * 0.1;
    return { min, max: paddedMax };
  } else {
    // For regular areas, find global min/max across all series
...
// Generate area path with baseline or stacked
const generateAreaPath = (key) => {
  if (props.data.length < 2) return '';
  
  // Generate the top part of the area (same as the line path)
  const topPath = generateLinePath(key);
  if (!topPath) return '';
  
  // Create the closing part of the path (bottom)
  let bottomPath = '';
  
  if (props.stacked) {
    // For stacked areas, calculate the baseline for this series
    // based on the sum of all series below it
    const keyIndex = props.keys.indexOf(key);
    
    // If this is the first key or not stacked, use the x-axis as baseline
    if (keyIndex === 0) {
      // Just connect to the x-axis at the end and beginning
      const lastX = xScale(props.data.length - 1);
      const firstX = xScale(0);
      bottomPath = ` L ${lastX},${yScale(0)} L ${firstX},${yScale(0)} Z`;
    } else {
      // Calculate the baseline from previous series
      const baselineKeys = props.keys.slice(0, keyIndex);
      
      // Reverse through the data points
      for (let i = props.data.length - 1; i >= 0; i--) {
        const x = xScale(i);
        
        // Sum of all values for series below this one
        const baseValue = baselineKeys.reduce((sum, baseKey) => {
          return sum + (props.data[i][baseKey] || 0);
        }, 0);
        
        const y = yScale(baseValue);
        
        if (i === props.data.length - 1) {
          // Start point for the bottom path
          bottomPath += ` L ${x},${y}`;
        } else {
          if (props.curveType === 'curved') {
            // Curved line for the bottom (mirroring the top path's curve style)
            const nextItem = props.data[i + 1];
            const nextX = xScale(i + 1);
            
            // Calculate the y coordinate for the next point on the baseline
            const nextBaseValue = baselineKeys.reduce((sum, baseKey) => {
              return sum + (nextItem[baseKey] || 0);
            }, 0);
            
            const nextY = yScale(nextBaseValue);
            ...
            // Control points
            const cp1x = x + dx * cpTension;
            const cp1y = y;
            const cp2x = nextX - dx * cpTension;
            const cp2y = nextY;
            
            // We're going backwards, so the control points are swapped compared to the top path
            bottomPath += ` C ${cp2x},${cp2y} ${cp1x},${cp1y} ${x},${y}`;
          } else {
            // Straight line
            bottomPath += ` L ${x},${y}`;
          }
    ...
    // For regular areas, just connect to the x-axis
    const lastX = xScale(props.data.length - 1);
    const firstX = xScale(0);
    bottomPath = ` L ${lastX},${yScale(0)} L ${firstX},${yScale(0)} Z`;
  }
...
// Get fill color for area with reduced opacity
const getAreaFill = (key) => {
